...
def proc( iproc, icomm, calc_num, integrals, calcs=None, N=80 ):
    x, w = weights_abscissas( N )
    if not iproc == 0:
        calcs = np.zeros( ( calc_num, 2 ), dtype=int )
...
def single( calc_num, integrals, N=80 ):
    x, w = weights_abscissas( N )
    calcs = np.zeros( ( calc_num, 2 ), dtype=int )
    for i in range( calc_num ):
        calcs[ i, 0 ] = i + 1
...
if iproc == root_process:
    if nproc == 1:
        single( calc_num, integrals )
    else:
        calcs = split_tasks( nproc, calc_num )
        calcs = icomm.bcast( calcs, root = 0 )
        integrals = proc( iproc, icomm, calc_num, integrals, calcs=calcs )
else:
